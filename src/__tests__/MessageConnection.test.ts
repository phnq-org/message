...
import { MessageType, type ResponseMessage } from "../MessageTransport";
...
const clientConnection = new MessageConnection<string, string>(
  ...
  describe("Conversation Summaries", (): void => {
    ...
    it("should yield expected summaries that agree client vs. server", async (): Promise<void> => {
      ...
      if (serverConvSummary && clientConvSummary) {
        ...
        expect(
          serverConvSummary.responses.map(({ message }): ResponseMessage<string> => message),
        ).toEqual(
